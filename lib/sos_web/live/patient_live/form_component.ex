defmodule SosWeb.PatientLive.FormComponent do
  ...
  def update(%{patient: patient} = assigns, socket) do
    changeset = CommandCenter.change_patient(patient)
  ...
  def handle_event("validate", %{"patient" => patient_params}, socket) do
    changeset =
      ...
      |> CommandCenter.change_patient(patient_params)
  ...
  defp save_patient(socket, :edit, patient_params) do
    case CommandCenter.update_patient(socket.assigns.patient, patient_params) do
      {:ok, _patient} ->
        {:noreply,
         socket
